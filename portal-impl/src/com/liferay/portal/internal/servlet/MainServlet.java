		...
		Thread reindexThread = new Thread(
			() -> {
				try {
					...
					options.setLocation("http://192.168.153.30:8080");
					...
					public void run() {
						try {
							reindexMethod1.invoke(
								backgroundTaskExecutor1, null, companyIds,
								"full");
						}
				...
				thread1.setName(
					"ReindexIndexReindexerBackgroundTaskExecutor-runner");
					...
					public void run() {
						try {
							reindexMethod2.invoke(
								backgroundTaskExecutor2, null, companyIds,
								"full");
						}
						catch (ReflectiveOperationException
									reflectiveOperationException) {
				...
				thread2.setName("ReindexPortalBackgroundTaskExecutor-runner");
