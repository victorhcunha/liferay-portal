/**
 * SPDX-FileCopyrightText: (c) 2025 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

declare module '@ckeditor/ckeditor5-adapter-ckfinder/dist/index.js' {
	export * from '@ckeditor/ckeditor5-adapter-ckfinder';
}
declare module '@ckeditor/ckeditor5-alignment/dist/index.js' {
	export * from '@ckeditor/ckeditor5-alignment';
}
declare module '@ckeditor/ckeditor5-autoformat/dist/index.js' {
	export * from '@ckeditor/ckeditor5-autoformat';
}
declare module '@ckeditor/ckeditor5-autosave/dist/index.js' {
	export * from '@ckeditor/ckeditor5-autosave';
}
declare module '@ckeditor/ckeditor5-basic-styles/dist/index.js' {
	export * from '@ckeditor/ckeditor5-basic-styles';
}
declare module '@ckeditor/ckeditor5-block-quote/dist/index.js' {
	export * from '@ckeditor/ckeditor5-block-quote';
}
declare module '@ckeditor/ckeditor5-bookmark/dist/index.js' {
	export * from '@ckeditor/ckeditor5-bookmark';
}
declare module '@ckeditor/ckeditor5-case-change/dist/index.js' {
	export * from '@ckeditor/ckeditor5-case-change';
}
declare module '@ckeditor/ckeditor5-ckbox/dist/index.js' {
	export * from '@ckeditor/ckeditor5-ckbox';
}
declare module '@ckeditor/ckeditor5-ckfinder/dist/index.js' {
	export * from '@ckeditor/ckeditor5-ckfinder';
}
declare module '@ckeditor/ckeditor5-clipboard/dist/index.js' {
	export * from '@ckeditor/ckeditor5-clipboard';
}
declare module '@ckeditor/ckeditor5-cloud-services/dist/index.js' {
	export * from '@ckeditor/ckeditor5-cloud-services';
}
declare module '@ckeditor/ckeditor5-code-block/dist/index.js' {
	export * from '@ckeditor/ckeditor5-code-block';
}
declare module '@ckeditor/ckeditor5-core/dist/index.js' {
	export * from '@ckeditor/ckeditor5-core';
}
declare module '@ckeditor/ckeditor5-document-outline/dist/index.js' {
	export * from '@ckeditor/ckeditor5-document-outline';
}
declare module '@ckeditor/ckeditor5-easy-image/dist/index.js' {
	export * from '@ckeditor/ckeditor5-easy-image';
}
declare module '@ckeditor/ckeditor5-editor-balloon/dist/index.js' {
	export * from '@ckeditor/ckeditor5-editor-balloon';
}
declare module '@ckeditor/ckeditor5-editor-classic/dist/index.js' {
	export * from '@ckeditor/ckeditor5-editor-classic';
}
declare module '@ckeditor/ckeditor5-editor-decoupled/dist/index.js' {
	export * from '@ckeditor/ckeditor5-editor-decoupled';
}
declare module '@ckeditor/ckeditor5-editor-inline/dist/index.js' {
	export * from '@ckeditor/ckeditor5-editor-inline';
}
declare module '@ckeditor/ckeditor5-editor-multi-root/dist/index.js' {
	export * from '@ckeditor/ckeditor5-editor-multi-root';
}
declare module '@ckeditor/ckeditor5-email/dist/index.js' {
	export * from '@ckeditor/ckeditor5-email';
}
declare module '@ckeditor/ckeditor5-emoji/dist/index.js' {
	export * from '@ckeditor/ckeditor5-emoji';
}
declare module '@ckeditor/ckeditor5-engine/dist/index.js' {
	export * from '@ckeditor/ckeditor5-engine';
}
declare module '@ckeditor/ckeditor5-enter/dist/index.js' {
	export * from '@ckeditor/ckeditor5-enter';
}
declare module '@ckeditor/ckeditor5-essentials/dist/index.js' {
	export * from '@ckeditor/ckeditor5-essentials';
}
declare module '@ckeditor/ckeditor5-export-inline-styles/dist/index.js' {
	export * from '@ckeditor/ckeditor5-export-inline-styles';
}
declare module '@ckeditor/ckeditor5-find-and-replace/dist/index.js' {
	export * from '@ckeditor/ckeditor5-find-and-replace';
}
declare module '@ckeditor/ckeditor5-font/dist/index.js' {
	export * from '@ckeditor/ckeditor5-font';
}
declare module '@ckeditor/ckeditor5-format-painter/dist/index.js' {
	export * from '@ckeditor/ckeditor5-format-painter';
}
declare module '@ckeditor/ckeditor5-fullscreen/dist/index.js' {
	export * from '@ckeditor/ckeditor5-fullscreen';
}
declare module '@ckeditor/ckeditor5-heading/dist/index.js' {
	export * from '@ckeditor/ckeditor5-heading';
}
declare module '@ckeditor/ckeditor5-highlight/dist/index.js' {
	export * from '@ckeditor/ckeditor5-highlight';
}
declare module '@ckeditor/ckeditor5-horizontal-line/dist/index.js' {
	export * from '@ckeditor/ckeditor5-horizontal-line';
}
declare module '@ckeditor/ckeditor5-html-embed/dist/index.js' {
	export * from '@ckeditor/ckeditor5-html-embed';
}
declare module '@ckeditor/ckeditor5-html-support/dist/index.js' {
	export * from '@ckeditor/ckeditor5-html-support';
}
declare module '@ckeditor/ckeditor5-icons/dist/index.js' {
	export * from '@ckeditor/ckeditor5-icons';
}
declare module '@ckeditor/ckeditor5-image/dist/index.js' {
	export * from '@ckeditor/ckeditor5-image';
}
declare module '@ckeditor/ckeditor5-indent/dist/index.js' {
	export * from '@ckeditor/ckeditor5-indent';
}
declare module '@ckeditor/ckeditor5-language/dist/index.js' {
	export * from '@ckeditor/ckeditor5-language';
}
declare module '@ckeditor/ckeditor5-line-height/dist/index.js' {
	export * from '@ckeditor/ckeditor5-line-height';
}
declare module '@ckeditor/ckeditor5-link/dist/index.js' {
	export * from '@ckeditor/ckeditor5-link';
}
declare module '@ckeditor/ckeditor5-list/dist/index.js' {
	export * from '@ckeditor/ckeditor5-list';
}
declare module '@ckeditor/ckeditor5-markdown-gfm/dist/index.js' {
	export * from '@ckeditor/ckeditor5-markdown-gfm';
}
declare module '@ckeditor/ckeditor5-media-embed/dist/index.js' {
	export * from '@ckeditor/ckeditor5-media-embed';
}
declare module '@ckeditor/ckeditor5-mention/dist/index.js' {
	export * from '@ckeditor/ckeditor5-mention';
}
declare module '@ckeditor/ckeditor5-merge-fields/dist/index.js' {
	export * from '@ckeditor/ckeditor5-merge-fields';
}
declare module '@ckeditor/ckeditor5-minimap/dist/index.js' {
	export * from '@ckeditor/ckeditor5-minimap';
}
declare module '@ckeditor/ckeditor5-page-break/dist/index.js' {
	export * from '@ckeditor/ckeditor5-page-break';
}
declare module '@ckeditor/ckeditor5-paragraph/dist/index.js' {
	export * from '@ckeditor/ckeditor5-paragraph';
}
declare module '@ckeditor/ckeditor5-paste-from-office/dist/index.js' {
	export * from '@ckeditor/ckeditor5-paste-from-office';
}
declare module '@ckeditor/ckeditor5-paste-from-office-enhanced/dist/index.js' {
	export * from '@ckeditor/ckeditor5-paste-from-office-enhanced';
}
declare module '@ckeditor/ckeditor5-remove-format/dist/index.js' {
	export * from '@ckeditor/ckeditor5-remove-format';
}
declare module '@ckeditor/ckeditor5-restricted-editing/dist/index.js' {
	export * from '@ckeditor/ckeditor5-restricted-editing';
}
declare module '@ckeditor/ckeditor5-select-all/dist/index.js' {
	export * from '@ckeditor/ckeditor5-select-al';
}
declare module '@ckeditor/ckeditor5-show-blocks/dist/index.js' {
	export * from '@ckeditor/ckeditor5-show-blocks';
}
declare module '@ckeditor/ckeditor5-slash-command/dist/index.js' {
	export * from '@ckeditor/ckeditor5-slash-command';
}
declare module '@ckeditor/ckeditor5-source-editing/dist/index.js' {
	export * from '@ckeditor/ckeditor5-source-editing';
}
declare module '@ckeditor/ckeditor5-source-editing-enhanced/dist/index.js' {
	export * from '@ckeditor/ckeditor5-source-editing-enhanced';
}
declare module '@ckeditor/ckeditor5-special-characters/dist/index.js' {
	export * from '@ckeditor/ckeditor5-special-characters';
}
declare module '@ckeditor/ckeditor5-style/dist/index.js' {
	export * from '@ckeditor/ckeditor5-style';
}
declare module '@ckeditor/ckeditor5-table/dist/index.js' {
	export * from '@ckeditor/ckeditor5-table';
}
declare module '@ckeditor/ckeditor5-template/dist/index.js' {
	export * from '@ckeditor/ckeditor5-template';
}
declare module '@ckeditor/ckeditor5-typing/dist/index.js' {
	export * from '@ckeditor/ckeditor5-typing';
}
declare module '@ckeditor/ckeditor5-ui/dist/index.js' {
	export * from '@ckeditor/ckeditor5-ui';
}
declare module '@ckeditor/ckeditor5-undo/dist/index.js' {
	export * from '@ckeditor/ckeditor5-undo';
}
declare module '@ckeditor/ckeditor5-upload/dist/index.js' {
	export * from '@ckeditor/ckeditor5-upload';
}
declare module '@ckeditor/ckeditor5-utils/dist/index.js' {
	export * from '@ckeditor/ckeditor5-utils';
}
declare module '@ckeditor/ckeditor5-watchdog/dist/index.js' {
	export * from '@ckeditor/ckeditor5-watchdog';
}
declare module '@ckeditor/ckeditor5-widget/dist/index.js' {
	export * from '@ckeditor/ckeditor5-widget';
}
declare module '@ckeditor/ckeditor5-word-count/dist/index.js' {
	export * from '@ckeditor/ckeditor5-word-count';
}
