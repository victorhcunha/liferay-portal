			...
			<c:if test="<%= baseAddressCheckoutStepDisplayContext.hasViewCommerceAddressesPermission() %>">
				<aui:select label='<%= "choose-" + baseAddressCheckoutStepDisplayContext.getTitle() %>' name="commerceAddress" onChange='<%= liferayPortletResponse.getNamespace() + "selectAddress();" %>' wrapperCssClass="commerce-form-group-item-row form-group-item">
					...
					%>
						...
						<aui:option data-city="<%= commerceAddress.getCity() %>" data-country="<%= String.valueOf(commerceAddress.getCountryId()) %>" data-name="<%= commerceAddress.getName() %>" data-phone-number="<%= commerceAddress.getPhoneNumber() %>" data-region="<%= String.valueOf(commerceAddress.getRegionId()) %>" data-street-1="<%= commerceAddress.getStreet1() %>" data-street-2="<%= Validator.isNotNull(commerceAddress.getStreet2()) ? commerceAddress.getStreet2() : StringPool.BLANK %>" data-street-3="<%= Validator.isNotNull(commerceAddress.getStreet3()) ? commerceAddress.getStreet3() : StringPool.BLANK %>" data-subtype="<%= commerceAddress.getSubtype() %>" data-type="<%= commerceAddress.getType() %>" data-zip="<%= commerceAddress.getZip() %>" label="<%= commerceAddress.getName() %>" selected="<%= commerceAddressId == commerceAddress.getCommerceAddressId() %>" value="<%= commerceAddress.getCommerceAddressId() %>" />
