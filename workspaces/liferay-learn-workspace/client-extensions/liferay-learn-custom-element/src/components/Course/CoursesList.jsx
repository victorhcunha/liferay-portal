        ...
        {courses && courses.length > 0 && (
          <>
            {courses.map((course, index) => {
              return (
                <SectionCard
                  description={!course.r_module_c_course.description ? "" : course.r_module_c_course.description}
                  expertise={course.r_module_c_course.level.name}
                  index={index}
                  key={course.r_module_c_course.id}
                  link={`/l/${course.lesson[0].id}`}
                  personas={course.r_module_c_course.audience}
                  title={!course.r_module_c_course.title ? "" : course.r_module_c_course.title}
                />
              );
            })}
          </>
